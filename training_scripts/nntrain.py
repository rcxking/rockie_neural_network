...
Last Updated: Bryant Pong: 5/27/15 - 7:03 PM  
'''
...
def train():
	...
	l_conv4 = lasagne.layers.Conv2DLayer(
		...
		nonlinearity=lasagne.nonlinearities.rectify,
		W=lasagne.init.HeNormal(gain='relu'))
	l_pool4 = lasagne.layers.MaxPool2DLayer(l_conv4, pool_size=(2,2))
	l_conv5 = lasagne.layers.Conv2DLayer(
		...
		nonlinearity=lasagne.nonlinearities.rectify,
		W=lasagne.init.HeNormal(gain='relu'))
	l_pool4 = lasagne.layers.MaxPool2DLayer(l_conv5, pool_size=(2,2))
	...
	while epoch < epochs:
		...
		if batchIdx >= X.shape[0]:
			...
			print("Percent correct: " + str(float(numCorrect) / X.shape[0]) + " for Epoch: " + str(epoch)) 
			print("Done validating neural net")			
			
			epoch += 1
			batchIdx = 0

			if float(numCorrect) / X.shape[0] > highestAccuracy:
				print("This Epoch has the highest accuracy!")
				highestAccuracy = float(numCorrect) / X.shape[0]
