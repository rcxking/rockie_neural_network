...
Last Updated: Bryant Pong: 6/5/15 - 5:33 PM
'''
...
def train(iter_funcs, dataset, batch_size=BATCH_SIZE):
	...
	for epoch in itertools.count(1):
		...
		for b in range(num_batches_train):
			...
			'''	
			print("Now printing images used in batch: " + str(b))
			for img in dataset['X_train'][slice(b * batch_size, (b+1)*batch_size)]:
				nextImg = img.reshape(imgHeight, imgWidth, 3)
				plt.imshow(nextImg)
				plt.show()
			'''	
